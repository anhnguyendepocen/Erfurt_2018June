...
# Code to knit slides
xaringan::inf_mr('_sessions/D2S2_Statistics/Statistics.Rmd')
```
...
Virtually all statistical functions try to model a dependent variable (y) as a function of one or more independent variables (x1, x2, ...).

In R, those variables should all be in a dataframe.
...
The three most important elements to any statistical function are the function name `STAT_FUN`, a `formula` specifying the variables and a dataframe `data` containing the data
...
Use the statistical function `glm` and model income (dependent) as a function of `sex`, `age` and `height` (independent)

```{r}
my_glm <- glm(formula = income ~ age + education,
              data = baselers)
```


```{r, echo = FALSE}
knitr::include_graphics("../_image/formula_description.png")
```
...
```{r, echo = FALSE}
knitr::include_graphics("../_image/glm_help.jpg")
```
...
```{r, echo = FALSE}
baselers %>% 
  select(food, income, alcohol, tattoos) %>%
  slice(1:5) %>% 
...
<br>
$$\Large food = \beta_{0} + \beta_{1} \times income + \epsilon$$
]

.pull-right55[

- Function: `glm()`
- Formula: `formula = food ~ income`
- Data: `data = ChickWeight`

```{r}
my_glm <- glm(formula = food ~ income,
              data = baselers)
