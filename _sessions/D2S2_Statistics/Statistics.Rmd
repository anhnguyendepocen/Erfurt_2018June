...
The two most important arguments to any statistical function are `formula` and `data`
...
<br>
<font size = 6>formula = y ~ x1 + x2 + ...</font><br>
...
Use the statistical function `STAT_FUN` and model income (dependent) as a function of sex, age and height (independent)

```{r, eval = FALSE}
# Load STAT_PACKAGE containing STAT_FUN
library(STAT_PACKAGE)

STAT_FUN(formula = income ~ sex + age + height,
         data = baselers)
