...
```{r, echo = FALSE}
knitr::opts_chunk$set(comment = NA, fig.width = 6, fig.height = 6, echo = FALSE, eval = TRUE)
```
...
4. How many houses were on the waterfront (`waterfront`) and how many were not? (Hint: use `table()`)

```{r}
table(kc_house$waterfront)
```

*Note: For all remaining analyses, you will be conducting statistical tests that return statistical objects. For each question, assign the result of the statistical test to an apporpriately named object, and then try exploring the object*


### T tests with t.test()

6. Do houses on the waterfront sell for more than those not on the waterfront? Answer this by creating an object called `water_htest` which contains the result of the appropriate t-test using `t.test()`
...
7. Print your `water_htest` object to see summary results

8. Look at the names of the `water_htest` with `names()`. What are the named elements of the object?

9. Using the `$` operator, print the exact test statistic of the t-test.

10. Again using the `$` operator, print the exact p-value of the test.

11. Use the `tidy()` function to return a dataframe containing the main results of the test.
