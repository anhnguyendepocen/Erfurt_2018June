...
author: "The R Bootcamp @ Erfurt June 2018"
...
Running code in parallel requires using a different programming style, but not by much. To make use of parallel execution, we must split the data into parcels, often called jobs, and specify a function build to process indvidiual parcels, often called a worker. Once both of them are specified, we can use the functions in R's `parallel` package to assign jobs to workers and collect the results. Let's begin by dealníng with the two ingredients, the jobs and the worker. In this section, we will be looking at a very simple task, to compute the means of every column in a matrix. 
