...
ML models are always evaluated based on their ability to *predict new, unseen data*.

U

<b> 1) Validation set</b>

Train on one or more historical dataset(s) (i.e.; previous year's stock changes)

Test on new data (i.e.; what happens this year)

<b> 2) K-Fold Cross Validation </b>

Uses only 

Split data into <i>X%</i> training and <i>1 - X%</i> test data.

Fit model to training data and evaluate prediction accuracy on test data
...
## Ridge Regression with `elasticnet::glmnet()`
...
Regularised ridge regression using the `elasticnet::glmnet()` function

```{r, eval = FALSE}
library(elasticnet)

# glm() function for regression
glmnet(formula = y ~.,     # Formula
...
```{r, echo = FALSE, eval = TRUE, out.width = "80%", fig.align = 'center'}
knitr::include_graphics("../_image/caretlogo.jpeg")
```
...
Use anye of hundreds of different ML algorithms by changing one "string' (not line!) to use a different model
